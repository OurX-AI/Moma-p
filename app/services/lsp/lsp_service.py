...
from .models import LspClientState,LspServerInfo
...
class CodeLSPService:
    ...
    async def _resolve_repo_root(repo_id:str)->str:
        """解析工作区根目录。coding Agent 传入 workspace_path；兼容旧调用把绝对目录当 root。"""
        ...
        if cached and Path(cached).is_dir():
            return cached
        as_path=Path(rid).expanduser()
        if as_path.is_dir():
            root=str(as_path.resolve())
